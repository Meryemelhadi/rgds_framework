...
// DebugBreak();
		...
		switch ($layout)
		{
			case "rows":
				...
				{
					$res .= '<tr><td><input class="radio" type="radio" name="___'.$fname.'" value="'.$k.'"'.
							(isset($sel[$k]) ? " checked=\"true\"" : "").'/></td><td></td></tr>';
				}
				return $res."</table>";
			case "tree":
			...
			default:
				...
				foreach (array_keys($listArray) as $k) 
				{
					$res .= '<input class="radio" type="radio" name="___'.$fname.'" value="'.$k.'"'.
