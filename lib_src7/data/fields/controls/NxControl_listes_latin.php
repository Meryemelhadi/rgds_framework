...
class NxControl_listes_latin
{
	...
	function toForm($value,&$f)
	{
		...
		if (isset($value))
		{
			...
			{
				$sel[$option]=true;
				$sel[preg_replace('/'.chr(146).'/','&rsquo;',htmlspecialchars($option,ENT_QUOTES))]=true;
				$sel[html_entity_decode($option,ENT_QUOTES)]=true;
			}
		}
...
//			$option="<option value=\"$k\">".htmlentities($listArray[$k])."</option>";
			$option="<option value=\"$k\">".htmlentities(preg_replace('/&rsquo;/','\'',$listArray[$k]))."</option>";
			if (isset($sel[$k]))
			{
				$list1 .= $option;
...
<input type="button" onclick="return remove{$fun}('{$fname}',event)" value="-->"><br/>
<input type="button" onclick="return add{$fun}('{$fname}',event)" value="<--">
</td>
<td style="vertical-align:top">
$others<br/>
<select class="select" {$isMultiple} 
