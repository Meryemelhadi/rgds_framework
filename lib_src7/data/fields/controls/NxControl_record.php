...
// singleton
$GLOBALS['NxControl_record_level']=0;

class NxControl_record
{
...
//				$record->setPrefix("{$fname}_{$i}__");
	...
	{		
		...
		if (($tpl=$desc->getProperty('tpl',null,false))==null)
			if (($tpl=$desc->getProperty($fname.'.tpl',null,false))==null)
			{
		   		$GLOBALS['NxControl_record_level']++;
				$desc->setProperty('record_level',$GLOBALS['NxControl_record_level']);
...
EOH;
                ...
                if ($add=='yes' || $add=='true')
                {
					...
					$s = '<div class="fields_records_wrapper"><div class="fields_records_add" onclick="window.new'.$jsPattern.'(this)"><span>'.$addStr.'</span></div>
				...
				$desc->runView($view,array('page.records'=>$records,'fmethod'=>'toHTML','sfrecord.isNew'=>0,'sfrecord.parentId'=>$f->record->getPrefix()),'page.subform');
		...
		{
	   		$GLOBALS['NxControl_record_level']++;
			$ds->setProperty('record_level',$GLOBALS['NxControl_record_level']);
