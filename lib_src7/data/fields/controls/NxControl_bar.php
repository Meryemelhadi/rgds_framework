...
class NxControl_text
{
	...
	{
		return '<div class="ctrl_percentbar_cont" style="width:100%"><div class="ctrl_percentbar" style="background-color:blue;width:'.$value.'%">'.$value.'%</div></div>';
	}
	...
	function toForm(&$value,&$f)
	{
		...
		else
		{
			...
			if ($size <= $colsMaxBox)
			{
				...
				$res = "<input class=\"text form-control\" type=\"text\" name=\"".$fname."\" size=\"$size\" maxlength=\"$maxLength\" ".$style." value=\"".$inputVal."\"".$filterChars."$evh/>";
			}
			else // if($multiLines == "no" || (int)$size < 2 * $colsMaxBox)
			{
				...
				$res = "<input class=\"text form-control\" type=\"text\" name=\"".$fname."\" size=\"$colsMaxBox\" maxlength=\"$maxLength\" ".$style." value=\"".$inputVal."\"". $filterChars."$evh/>";
			}
		}
