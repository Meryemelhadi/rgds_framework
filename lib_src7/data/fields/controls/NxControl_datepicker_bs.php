...
class NxControl_datepicker_bs
{
	...
	function toForm(&$f,$controlName,$opts=null)
	{
		...
		{
			$s.='
	<link rel="stylesheet" href="/nx/controls/datepicker/datepicker_bs.css" />
	<script>
		...
		{
			ajax_require("/nx/controls/datepicker/datepicker_bs.js","datepicker");
		}
	</script>
';
		}
		...
		<input id="{$fId}" name="{$fname}" type="text" placeholder="jj/mm/aaaa" pattern="[0-9]{2}/[0-9]{2}/[0-9]{4}" value="{$inputVal}" class="form-control form-control-date" xreadonly="readonly"/>
		<span class="input-group-addon" id="{$fId}_handler"><i class="glyphicon glyphicon-calendar"></i></span>
		</div>
		...
		if (typeof ajax_require != "undefined")
		{
			ajax_onload(function(){ajax_onload(function(){
				try {
					new window.Control.DatePicker(document.getElementById('{$fId}'), { handler:document.getElementById('{$fId}_handler'),icon: false, locale:'{$locale}', timePicker:false, timePickerAdjacent: false, use24hrs: true });
				} catch(e) {
