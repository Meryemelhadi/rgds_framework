...
/** 
	Data field used to store criteria for a SQL Query on a field of type text choice.
	it stores:
	- relation type (ex. before, after, etc.)
	- value of relation as a list of values (tomorrow, today, etc.)
	
	NB. the actual values for these list must make sense for NxDB_Database::getWhereClause() 
*/
class NxDataField_text_choice_relation extends NxData_Field
{
	var $value;
	...
	function toForm($opts=null) 
	{
		...
		$f_rel=$desc_relation->createField(array(),'array',$this->error);
		
		$s='';
		$control =& $desc_relation->getControl($desc->getProperty('control_relation','select',false));
		if ($control!==null)
			$s.= $control->toForm($rel,$f_rel);
	...
	function _readFromForm(&$recData,&$errorCB)
	{
		...
		// todo: check this line
		$data=array();
		$f_rel=$desc_relation->createField($data,'array',$this->error);
		
		$control =& $desc_relation->getControl($desc->getProperty('control_relation','select',false));
		if ($control!==null)
			$rel = $control->readForm($recData,$f_rel);
		else
			$rel='=';
