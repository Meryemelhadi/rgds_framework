...
class NxDataField_records extends NxData_Field
{
	...
	{
		return NxData_Field::clone_it($record,$errorCB,false);
	}
	...
	function toForm($opts=null) 
	{
		...
		if (!$desc->isEdit()) 
		{
			// force sub controls to be html...
			global $FORCE_NO_EDIT;
			$editForceMode = $FORCE_NO_EDIT;
			$FORCE_NO_EDIT=true;

				$res = $this->toHTML()."<input type=\"hidden\" name=\"".$desc->getFormName()."\" value=\"".$this->value."\" />";			

			$FORCE_NO_EDIT=$editForceMode;
			return $res;
		}
