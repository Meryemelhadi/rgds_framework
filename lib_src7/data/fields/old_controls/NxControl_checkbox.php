...
class NxControl_checkbox
{
	...
	function toForm($value,&$f)
	{
		...
		switch ($layout)
		{
			case "rows":
				...
				{
					$i++;
					$res .= '<tr><td><input class="'.$itype.'" type="'.$itype.'" name="'.$fname.'[]" value="'.$k.'"'.
							(isset($sel[$k]) ? " checked=\"true\"" : "").'/></td><td class="checkbox_label"><label  for="'.$fname.$i.'">'.$listArray[$k].'</label></td></tr>';
				}
				return $res."</table>";
			case "columns":
				$res = "<table ".$style."><tr>";
				...
				{
					$i++;
					$res .= '<td><input class="'.$itype.'" type="'.$itype.'" name="'.$fname.'[]" value="'.$k.'"'.
							(isset($sel[$k]) ? " checked=\"true\"" : "").'/></td><td class="checkbox_label"><label  for="'.$fname.$i.'">'.$listArray[$k].'</label></td>';
				}
				return $res."</tr></table>";
			case "tree":
			...
			default:
				...
				{
					$i++;
					$res .= '<input class="'.$itype.'" type="'.$itype.'" id="'.$fname.$i.'" name="'.$fname.'[]" value="'.$k.'"'.
							(isset($sel[$k]) ? " checked=\"true\"" : "").'/><label  for="'.$fname.$i.'" class="checkbox_label">'.$listArray[$k].'</label>&nbsp;';
				}
				return $res."</span>";
		}			
	}
	...
	function _toFormTree($fname,&$listArray,&$sel,$style,$itype)
	{
		...
		foreach (array_keys($listArray) as $k) 
		{
			...
			if (substr($k,0,1)!='!')
			{
				$res .= '<input class="'.$itype.'" type="'.$itype.'" name="'.$fname.'[]" value="'.$k.'"'.
