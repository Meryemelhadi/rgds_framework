...
class NxControl_int_phone
{
	...
	function toForm($value,&$desc)
	{
		...
		// international code
		$res .= $this->_toFormIntCode($parts[0],$desc);
		...
		// local number
		$res .='&nbsp;<input onkeypress="keypress_phonenumber(event)"  class="other_text" id="phone_local_number" type="text" name="'.$desc->name."_num".'" value="'.$parts[2].'"/>';

		return $res."</span>";
	}

	function _toFormIntCode($value,&$desc)
	{
