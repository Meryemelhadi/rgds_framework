 ...
 * @version 0.99.1dev
 *
 * modified by Franck Yvetot <franck.yvetot@nxfrontier.com>
 */
class HTTP_WebDAV_Server 
{
    ...
    /**
     * do we want to add slashes at the end of directories?
     *
     * @var bool 
     */
    var $add_dir_slash;
     ...
     * @var string 
     */
    var $dav_powered_by = "NX-OFFICE-DAV-SERVER";
     ...
     * @param void
     */
//    function HTTP_WebDAV_Server($base_uri,$base_dir,$add_dir_slash=true) 
    function HTTP_WebDAV_Server($base_uri,$base_path_uri,$add_dir_slash=true) 
    {
        ...
        $this->base_path_uri = rtrim($base_path_uri,'/');
                
        $this->add_dir_slash = $add_dir_slash;
        ///FY
    }

    // }}}

    //! get actual path. Removes url prefix for the dav URL and returns the contant path uri.
    function getPath($path_uri) 
    {
    	if (!strncmp($this->base_path_uri,$path_uri,strlen($this->base_path_uri)))
    		return urldecode(substr($path_uri, strlen($this->base_path_uri)));
    	
    	nxError("bad path uri: [$path_uri] with base uri=[{$this->base_path_uri}]");
    	
    	return false;
    }
    
    //! map ressource path to uri (without host prefix)
    function getURIPath($path) 
    {
    	return $this->base_uri.implode('/', array_map('rawurlencode', explode('/', $path)));;
    }
    
    //! get required host and path for the file uri. Returns path.
    function getDestURI(&$host) 
    {
        ...
        return $this->getPath($url["path"]);
    }

    //! get host of the webserver
    ...
    function ServeRequest($path_uri=null) 
    {
        ...
        $uri.= "//$_SERVER[HTTP_HOST]".preg_replace('[/\]+','/',dirname($_SERVER['SCRIPT_NAME'])); //FY
        ...
        */
        ...
        if (empty($path_uri))
        	$path_uri=$_SERVER['PATH_INFO'];
        	
        $this->path_uri = $path_uri; // fy
        $this->uri = $this->getURIPath($path_uri);
        ...
        if (!$this->_check_auth()) {
            ...
            //
            // FY: protect it with https at least during authentication? Then
            // redirect to http?
        ...
        // set path
        $this->path = $this->_urldecode($path_uri);

        if (!strlen($this->path)) {
            header("Location: ".$this->base_uri); //FY
//            header("Location: ".$this->base_uri."/");
            exit;
        }

        // PHP stuff for managing http params (cf. PHP doc)
        if(ini_get("magic_quotes_gpc")) {
            $this->path = stripslashes($this->path);
        }
                
        // detect requested HTTP method names:
        ...
        { // method not found/implemented
            ...
            else 
            {
            	...
            	// FY: do we need to add support other for some methods or is the DAV client 
            	// flexible enough for coping with existing ones?
...
//fy            $href = $this->_slashify($_SERVER['SCRIPT_NAME'] . $path);
            // $href = $this->base_uri . urlencode($this->_slashify($path));
            $href = $this->getURIPath($path);
    ...
    function http_PROPPATCH() 
    {
        if($this->_check_lock_status($this->path)) {
            ...
            echo "<D:multistatus xmlns:D=\"DAV:\">\n";
            echo " <D:response>\n";
//fy            echo "  <D:href>".$this->_urlencode($_SERVER["SCRIPT_NAME"].$this->path)."</D:href>\n";
            echo "  <D:href>".$this->_urlencode($this->base_uri.$this->path)."</D:href>\n";
            ...
            if ($stat === false) {
                ...
                if (!empty($options["ranges"])) 
                {
                    ...
                    if (0 == fseek($stream, $range[0]["start"], SEEK_SET)) {
                        ...
                        if ($length>0 && !fwrite($stream, fread($options["stream"], $length))) {
                        	nxLog("can't read/write a file range");
                            $stat = "403 Forbidden"; 
                        }
                    } else {
                       	nxLog("range issue");
    ...
    function http_LOCK() 
    {
        ...
        if(empty($_SERVER['CONTENT_LENGTH']) && !empty($_SERVER['HTTP_IF'])) {
            ...
            //fy: reload info from lock db
            $lockinfo = $this->checkLock($this->path);
            $options["scope"] = $lockinfo["scope"];
            $options["type"]  = $lockinfo["type"];
            $options["owner"] = $lockinfo["owner"];
            $options['depth'] = $lockinfo["depth"];
            // $options['timeout'] = $lockinfo["expires"];
            $options["locktoken"] = $lockinfo["token"];

            // to do check update timeout works..?
            nxLog("Lock (update) info: token:$options[update] timeout:{$options['timeout']} s:$options[scope] t:$options[type] o:$options[owner]",'DAV'); 
            // /fy
            ...
            $options["owner"] = $lockinfo->owner;
            nxLog("Lock info: s:$options[scope] t:$options[type] o:$options[owner]",'DAV'); 
    ...
    // fy: get lock info for propfind
    function supportedLock($path) 
    {
        $options = Array();
        $options["path"] = $path;
        
        // get lock info
        $stat = $this->checkLock($path);
        if(!is_array($stat))
        	return '';

        // reload info from lock db
        $options["scope"] = $stat["scope"];
        $options["type"]  = $stat["type"];
        $options["owner"] = $stat["owner"];
        $options['depth'] = $stat["depth"];
        $options['timeout'] = $stat["expires"];
        $options["locktoken"] = $stat["token"];
    ...
    function http_UNLOCK() 
    {
        ...
        nxLog("Unlock info: token:$options[token]",'DAV'); 
   ...
   function _copymove($what) 
    {
        ...
        // get dest path and host
        $http_host='';
        $destPath=$this->getDestURI($http_host);

        // get webserver host
        $http_header_host=$this->getServerHost();
       
        nxLog(" copy move: from [host={$http_header_host},{$options["path"]}] to [host={$http_host} target={$destPath}]",'DAV');
        
        if ($http_host == $http_header_host && $destPath!==false)
       {
            $options["dest"] = $destPath;
       	
            nxLog("options[dest]=".$options["dest"]);
            ...
            nxLog("options[dest_url]=".$options["dest_url"]);
        }
...
//        	nxlog("CONDITIONS: {$_SERVER['HTTP_IF']}",'DAV');
                ...
                foreach($conditions as $condition) {
                    ...
                    if (!strncmp($condition, "<opaquelocktoken:", strlen("<opaquelocktoken"))) {
                        if (!ereg("^<opaquelocktoken:[[:xdigit:]]{8}-[[:xdigit:]]{4}-[[:xdigit:]]{4}-[[:xdigit:]]{4}-[[:xdigit:]]{12}>$", $condition)) {
           					nxError("Error bad opaquelocktoken format",'DAV');
                            return false;
                        }
                    }
        ...
        if (method_exists($this, "checkLock")) {
            ...
            if (is_array($lock) && count($lock)) {
                ...
                if (!strstr($_SERVER["HTTP_IF"], $lock["token"])) {
                    ...
                    {
       					nxLog("lock status: isnt owned..",'DAV');
                        return false;
                    }
                }
            }
