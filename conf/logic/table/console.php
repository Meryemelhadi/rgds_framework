...
class table_console extends NxApplication
{
	...
	*/
	function run()
	{
		...
		$this->setProperty("viewRecord.url",$this->getUrl("","dispRec"));
		...
		{	
			...
			case "dispRec":
				$this->step='view';
				$this->startOp();
				$this->runLogic('table.displayRec.process');
				$this->finish();
				break;
			/*
			case "editRec":
			...
			case 'deleteRec':
				switch($this->step)
				{
					case 'process':
						...
						if ($this->checkPreviousStep('confirm','display',null,'page.message'))
						{
							// step 4: store record in db
							if ($this->runLogic('table.deleteRec.process'))
								$this->redirectTo('page.to_page');							
							$this->finish();
							return true;
						}
			...
			// case "console":
			default:
				$this->setReturnPage($this->getUrl());
